...
// Modal component yahan import karo—yeh wahi ho jo tumne pehle bana rakha hai
import RoleSelectModal from "./Recruiter-Section/Modal"; // Make sure path sahi ho
    ...
    <>
      <nav className="flex justify-between items-center px-10 py-4 bg-white sticky top-0 z-50 border-b border-gray-300">
          ...
          {navLinks.map(({ name, to }) => (
            <li key={name} className="relative group">
              ...
              >
                {name}
                {/* Underline animation */}
                {/* Underline motion code here if needed */}
              </NavLink>
            </li>
          ))}
        </ul>
        ...
        <div className="flex gap-5">
          ...
          >
            <button
              onClick={() => setModalOpen(true)} // Modal yahan se open hoga
      ...
      {/* Modal is displayed conditionally */}
      <RoleSelectModal open={modalOpen} onClose={() => setModalOpen(false)} />
    </>
  );
}
